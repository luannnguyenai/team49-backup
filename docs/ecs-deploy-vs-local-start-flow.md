...
| 9. Seed canonical/product shell | Tự chạy `scripts/seed.py` nếu `learning_units` rỗng tại [start.sh](/D:/VSCODE/VINAI/A20-App-049/start.sh:253) và [seed.py](/D:/VSCODE/VINAI/A20-App-049/scripts/seed.py:51) | Không chạy mặc định trong deploy; chỉ có bootstrap one-off nếu bật `run_bootstrap=true` tại [deploy-ecs-prod.yml](/D:/VSCODE/VINAI/A20-App-049/.github/workflows/deploy-ecs-prod.yml:197) | Local tự nạp dữ liệu; ECS mặc định giả định DB đã có data |
| 10. Seed lectures | Tự chạy `scripts/seed_lectures.py` nếu bảng `lectures` rỗng tại [start.sh](/D:/VSCODE/VINAI/A20-App-049/start.sh:267) | Không có bước tương ứng mặc định | Đây là một nguồn chênh lệch dữ liệu lớn giữa local và ECS |
| 11. Schema v2 sync/backfill/validate | Chạy import canonical artifacts, backfill, validate, parity check tại [start.sh](/D:/VSCODE/VINAI/A20-App-049/start.sh:278) | Không có trong deploy workflow | Local startup làm data-shaping nhiều hơn ECS rất nhiều |
| 12. Admin/demo accounts | Tạo account bằng `src.scripts.create_seed_accounts` tại [start.sh](/D:/VSCODE/VINAI/A20-App-049/start.sh:397) | Không có bước tương ứng | Local có thể usable ngay; ECS có thể thiếu account nếu chưa provision từ trước |
