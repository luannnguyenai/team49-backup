...
- **Frontend UI rebuild + Clerk auth cutover** (Đức): Design tokens (phases 1-4), Vietnamese copy toàn app, lesson sidebar, TopNav routes, tutor hub layout (enrolled courses, recommended courses). Fix alembic merge heads. Clerk auth backend cutover (user sync + webhook, Phases 5-13). Hybrid course recommender (19-22/04).
