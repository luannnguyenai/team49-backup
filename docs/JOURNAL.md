...
- **Hybrid branch integration — backend hardening** (Rin): Gộp `db-review` vào `course-first` qua nhánh hybrid (18/04) — repository layer cho auth/history/recommendation/assessment, `QuestionSelector` service, `DomainError` exception pattern, `pgvector` extension migration, audit table `mastery_history`. Redis-backed rate limiting, token denylist, logout revoke endpoint, fix startup/CORS/config, forgot-password flow, lecture-aware tutor scope guard, course platform decouple/refactor.
- **DB foundation backend (Phase 1-5)** (Đức): DB foundation cleanup TDD (Phase 1), Repository layer data access (Phase 2), QuestionSelector service (Phase 3a), services raise DomainError (Phase 3b), Redis rate limiting + token denylist (Phase 4), Docker cleanup + mastery_history audit table (Phase 5) (17/04).
- **Frontend UI rebuild + Tutor Hub** (Đức): Design tokens (phases 1-4), Vietnamese copy toàn app, tutor hub layout đầy đủ 5 tính năng (enrolled courses grid, recommended courses, resume card từ sessionStorage, course catalog, empty state), lesson sidebar, TopNav routes, restore learn landing page + topic content page, restore nav items (AI Tutor, Học), fix alembic merge heads × 2. Hybrid course recommender (19-22/04).
...
- **Full i18n translation Vi→En — 50+ components** (Đức): Convert toàn bộ app từ tiếng Việt sang tiếng Anh — assessment flow, course catalog, dashboard, history, in-context tutor, landing page, tất cả learning-path sub-components, learning-unit card/drawer/shell, module test, onboarding wizard (tất cả steps), quiz flow, roadmap, profile, tutor page, timeline board. Chuẩn hóa navigation names, tab names, copywriting (30/04).
...
- **Full UI Theme System Phases 0-5** (Đức): Phase 0 retint buttons, Phase 1 public CTA unify (landing CTA tests), Phase 2 auth pages refactor (LoginForm, RegisterForm, ForgotPasswordForm, AuthBackLink), Phase 3 decorative tokens, Phase 4 bloom badges + session migrate, Phase 5 admin chart palette (ChartCard, KpiCard, chart-theme). AgentChatPage theme, PublicTopNav, feedbackStyles.ts, globals.css, tailwind.config.ts. Test coverage cho tất cả theme phases (achievement-tier, bloom-badge, chart-theme, auth-pages-theme, decorative-tokens, landing-cta). Learning path UX improvement (04/05).
- **Reset password flow** (Đức): Implement complete reset password flow frontend + backend wiring — 15 iterations (05/05).
- **Admin dashboard + Langfuse** (Đức): Ship admin dashboard (AdminSidebar, KpiCard, admin-api.ts, admin.py, test_admin_routes.py), tutorSessionHistory.ts, authStore.ts. Setup Langfuse integration: traces, spans, env config, test_tutor_observability.py (02-03/05).
- **InContextTutor + agent UI** (Đức): Update InContextTutor.tsx, PrerequisiteSuggestionDialog.tsx, ReplanKnowledgeClaimStep.tsx, TopNav.tsx. Admin tooltips cho observability (07/05).
...
- **Bilingual localization pipeline** (Rin): Xây dựng toàn bộ i18n pipeline — `localized_content` schema, localization ingestion pipeline, runtime helper, localize canonical question/course/learning surfaces. Thêm frontend language switcher. Review và finalize toàn bộ Vietnamese translation cho question bank, course copy, overview sections. Stabilize localized learning path order (12/05).
- **External research mode** (Rin): Wiring đầy đủ external research flow — external citation manager, external search planning citations, web paper search mode, agent search mode selector, synthesize external search answers, render external citations inline. Add Semantic Scholar API feature flag, disable by default (12/05).
- **Agent guardrail & router improvements** (Rin): Externalize guardrail prompt sang config file. Externalize assistant help prompt. Fix language detection edge cases (refine input language detection). Guardrail prompt benchmark cho router variants. Fix allow safe assistant operations. Fix guardrail assistant followup context. Pin active citation for agentic followups (11-12/05).
...
- **Landing page redesign** (Luân): Cập nhật toàn bộ landing page theo sản phẩm production thực tế — nội dung mới, button dimensions chuẩn, bỏ social proof section chưa có data thật, thêm Redis error handling cho backend resilience (12-13/05).
...
- **Auth UX hardening** (Rin): Fix stale error hydration trong auth store (lỗi persisted error từ session cũ hiển thị sai khi load trang mới). Fix login/register stale error. Relax password schema validation cho demo accounts — giảm độ phức tạp để demo không bị friction với `DemoPass123!`. Thêm test coverage cho toàn bộ auth form edge cases (15/05).
- **RoadmapPlanner master-detail** (Rin): Migrate từ flat list sang master-detail layout với sidebar navigation. Unit tests cập nhật theo layout mới. Fix top nav logo destination (`/dashboard`). Fix learning drawer CTA state (15/05).
- **Agent route context** (Rin): Persist learning route context trong unit shell, pass route context từ agent page vào RAG pipeline, wire user learning context vào agent router. Route progress requests through agentic RAG. Thêm agent route context helper và test coverage (15/05).
- **Agent learner context & workload tools** (Rin): Thêm user learning context snapshots, learner context và lecture tools, register learner/lecture RAG tools. Add lecture summary aggregator. Support primary course workload extraction trong learning context. Update agentic RAG pipeline, tool nodes, graph service, guardrail router (15-16/05).
