...
- [ ] T022 Thêm `src/repositories/base.py` theo pattern của `db-review` làm nền cho repository layer của hybrid branch
- [ ] T023 Áp repository layer cho phần user/auth state, tạo hoặc cập nhật `src/repositories/*` tương ứng cho user/session/profile nếu vùng này đã DB-backed thật
- [ ] T024 Áp repository layer cho assessment/history/recommendation ở `src/repositories/*` chỉ khi logic đang chạm DB thật, không bọc giả các bootstrap services
