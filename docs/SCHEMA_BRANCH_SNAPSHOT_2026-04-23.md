...
## Update 2026-04-23 — Hard Canonical Cutover Complete

Từ snapshot này trở đi, có 4 thay đổi quan trọng:

1. Runtime production code trong `src/` đã cắt khỏi:
   - `modules`
   - `topics`
   ...
   - `mastery_history`
   - `learning_paths`
2. Legacy KG package `src/kg/*` đã bị gỡ khỏi runtime codebase.
3. Alembic head hiện là `20260423_drop_legacy`.
4. DB dev đã xác nhận các bảng legacy ở trên không còn tồn tại.

Hệ quả thực tế:

- assessment / quiz / module-test giờ chỉ chạy bằng canonical units/question bank/KP mastery
- planner runtime chỉ đọc canonical planner audit + `learner_mastery_kp`
...
Hiện runtime cutover đã hoàn tất cho các flow product chính. Các legacy allow-flags đã không còn cần thiết vì fallback tables đã bị drop khỏi DB.
...
- `question_bank` / `item_phase_map`
  - reader: `src/services/assessment_service.py:start_assessment`
  - flag: `read_canonical_questions_enabled`
  - input mới: `AssessmentStartRequest.canonical_unit_ids[]` + `phase`
  - legacy input `topic_ids[]` vẫn được giữ để không phá client cũ
  - selector mới:
    - `src/repositories/canonical_question_repo.py`
    - `src/services/canonical_question_selector.py`
  - rule quan trọng:
    - lecture/unit ownership lấy từ `question_bank.course_id`, `lecture_id`, `unit_id`, `source_ref`
    - assessor/phase ownership lấy từ `item_phase_map.phase`
    - KP evidence ownership lấy từ `item_kp_map`

- `learner_mastery_kp`
  - writer mới: `src/services/canonical_mastery_service.py`
  - gọi từ canonical assessment / quiz / module-test submit path
  - update KP qua `item_kp_map.item_id -> kp_id`
...
- `plan_history` / `rationale_log` / `planner_session_state` canonical branch
  ...
  - flag: `read_canonical_planner_enabled`
  - đọc:
    - `goal_preferences.selected_course_ids`
    - `learning_units.canonical_unit_id`
    - `unit_kp_map`
    - `learner_mastery_kp`
  - ghi audit với `rationale_log.learning_unit_id` thật thay vì `null`
...
Các cột bridge mới đã có ORM + Alembic:

- `courses.canonical_course_id`
  - business ID của course trong canonical artifact, ví dụ `CS224n`, `CS231n`.
- `learning_units.canonical_unit_id`
  - trỏ sang `units.unit_id`.
  - được backfill bằng `src/scripts/pipeline/backfill_product_canonical_links.py`.
- `sessions.canonical_phase`
  - phase assessor/planner canonical của session, ví dụ `placement`, `mini_quiz`.
- `interactions.canonical_item_id`
  - trỏ sang `question_bank.item_id`.
  - `interactions.question_id` đã nullable để canonical-only answer event không cần fabricate legacy `questions.id`.

Các migration liên quan:

- `alembic/versions/20260423_runtime_canonical_bridge_columns.py`
- `alembic/versions/20260423_nullable_interaction_question.py`

### Runtime parity / freeze guard
...
- `learning_units` chưa link `canonical_unit_id`
- `question_bank` thiếu `item_phase_map`
- `question_bank` thiếu `item_kp_map`
- canonical-only `interactions` thiếu `canonical_item_id`
...
- Runtime vẫn còn cần parity/backfill trước khi bật read flags trong môi trường thật:
  ...
  - backfill `canonical_course_id` / `canonical_unit_id`
  - chạy parity checker
- Runtime vẫn chưa có shared taxonomy hoàn chỉnh end-to-end giữa:
...
- Nếu mục tiêu là mastery/IRT/CDM tinh vi:
  - runtime đã có bootstrap update vào `learner_mastery_kp`, nhưng calibration thật vẫn cần interaction volume và IRT job riêng.
