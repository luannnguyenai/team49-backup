...
Hiện runtime cutover đã tiến thêm một lớp so với schema foundation ban đầu. Các path mới đều nằm sau feature flag, nên web cũ vẫn chạy theo legacy tables khi flag tắt.
...
- `learner_mastery_kp`
  ...
  - gọi từ canonical assessment submit khi:
    - `write_canonical_interactions_enabled=true`
    - `write_learner_mastery_kp_enabled=true`
    - answer payload có `canonical_item_id`
  - update KP qua `item_kp_map.item_id -> kp_id`
  - legacy topic-grain `_upsert_mastery_scores` vẫn tồn tại cho path cũ
