...
function levelLabel(level: PriorTopicLevel): string {
  if (level === "confident") return "Confident";
    ...
    <div className="space-y-5">
      <div>
        <p className="text-sm font-medium" style={{ color: "var(--text-primary)" }}>
          Confirm suggested AI topic groups
        </p>
        <p className="mt-1 text-xs leading-relaxed" style={{ color: "var(--text-muted)" }}>
          AI has already selected the groups that clearly match your description. Only uncertain
          groups appear here for you to decide on; this is still not counted as mastery.
        </p>
      </div>
      ...
      {!analysisFallback && (
        ...
        >
          Shortlist generated by {modelLabel ?? "AI reasoning model"}.
        </div>
      )}
      ...
      {visibleTopics.length === 0 ? (
        <div
          ...
          style={{ borderColor: "var(--border)", color: "var(--text-muted)" }}
        >
          There are no more groups that need confirmation. You can continue so the system can create the right placement.
        </div>
      ) : (
                ...
                <div className="flex items-start gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-xs" style={{ color: "var(--text-muted)" }}>
                      {topic.units.length} representative units for placement verification
                    </p>
                  </div>
                  <button
                    type="button"
      ...
      {knownUnitIds.length > 0 && (
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-xs font-medium text-primary-600">
            <p>Selected {knownUnitIds.length} representative units for placement verification.</p>
            <button
              type="button"
          ...
          {showSelectedUnits && (
            ...
            >
              <p className="mb-2 text-xs font-semibold" style={{ color: "var(--text-primary)" }}>
                Units currently included in placement
              </p>
              <div className="space-y-3">
