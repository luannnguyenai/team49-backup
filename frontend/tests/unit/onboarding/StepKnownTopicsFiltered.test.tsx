...
const cvTopics = buildPriorCandidateTopics({
  ...
  ...topic,
  summary: "Summary of CNNs, convolutional architectures, and common computer vision tasks.",
}));
...
describe("StepKnownTopicsFiltered", () => {
  ...
  it("renders only the AI-selected confirmation shortlist", () => {
    ...
    expect(screen.getByText("Shortlist generated by openai/gpt-5.4-mini.")).toBeInTheDocument();
  });
  ...
  it("lets the user inspect selected representative units", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "View selected units" }));
  ...
  it("lets the user select representative units", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Reviewed CNN architecture design" }));
    expect(useOnboardingStore.getState().knownUnitIds).toContain("u1");

    fireEvent.click(screen.getByRole("button", { name: "Not studied CNN architecture design" }));
  ...
  it("does not render assessment depth in topic confirmation", () => {
    ...
    expect(screen.queryByText("Assessment depth")).not.toBeInTheDocument();
  ...
  it("eye button reveals a summarized representative content preview", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Quick view CNN architecture design" }));
    expect(
      screen.getByText("Summary of CNNs, convolutional architectures, and common computer vision tasks."),
    ).toBeInTheDocument();
    expect(screen.queryByText("- Conv Basics")).not.toBeInTheDocument();
  });
