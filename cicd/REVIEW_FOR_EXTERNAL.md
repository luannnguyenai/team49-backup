...
jobs:
  changes:
    runs-on: [self-hosted]   # ← N3: self-hosted only
    steps:
...
`ci.yml:31` `runs-on: [self-hosted]`. If runner offline → deploy blocks at `needs: ci`. Recommend `ubuntu-latest` for the gating job, or set up backup runner.
