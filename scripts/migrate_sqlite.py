...
_SYNC_DATABASE_URL = settings.database_url.replace("+asyncpg", "").replace("postgresql://", "postgresql+psycopg2://")
...
def migrate():
    ...
    sl_cursor.execute("SELECT id, title, description, video_url, duration, created_at FROM lectures")
    for row in sl_cursor.fetchall():
        pg_db.add(Lecture(
            id=row[0], title=row[1], description=row[2], 
            video_url=row[3], duration=row[4], created_at=row[5]
        ))
    ...
    for row in sl_cursor.fetchall():
        pg_db.add(Chapter(
            lecture_id=row[0], title=row[1], summary=row[2], 
    ...
    for row in sl_cursor.fetchall():
        pg_db.add(TranscriptLine(
            lecture_id=row[0], start_time=row[1], end_time=row[2], content=row[3]
        ))

    # 4. QA History
    print("Migrating QA History...")
    sl_cursor.execute("SELECT lecture_id, question, answer, thoughts, current_timestamp, image_base64, created_at FROM qa_history")
    for row in sl_cursor.fetchall():
        pg_db.add(QAHistory(
            lecture_id=row[0], question=row[1], answer=row[2], 
            thoughts=row[3], current_timestamp=row[4], 
            image_base64=row[5], created_at=row[6]
        ))
