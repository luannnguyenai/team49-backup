...
def test_select_courses_defaults_multi_specialization_demo_to_cv_path():
    ...
    assert [course.slug for course in selected] == ["cs230", "cs231n"]


def test_select_courses_includes_deep_learning_foundation_for_nlp_path():
    cs230_course_id = uuid.uuid5(uuid.NAMESPACE_URL, "test-course-cs230")
    cs224n_course_id = uuid.uuid5(uuid.NAMESPACE_URL, "test-course-cs224n")
    cs231n_course_id = uuid.uuid5(uuid.NAMESPACE_URL, "test-course-cs231n")
    catalog = SyntheticCatalog(
    ...
    selected = _select_courses(catalog, ("cs224n",))
