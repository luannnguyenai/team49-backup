...
class OnboardingGoalIdsTests(unittest.TestCase):
    ...
    def test_onboarding_request_rejects_multiple_goal_ids(self):
        import pydantic
        from src.schemas.auth import OnboardingRequest
        with self.assertRaises(pydantic.ValidationError):
            OnboardingRequest(
    ...
    def test_merged_course_ids_deduplication(self):
        # If goal_ids contains "computer_vision" (→ [cs230, cs231n]) and selected_course_ids
        ...
        self.assertEqual(merged, ["cs231n", "cs230"])  # no duplicate
