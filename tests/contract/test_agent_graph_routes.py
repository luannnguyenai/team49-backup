...
async def test_agent_chat_commits_retry_context_for_router_unavailable_response():
    user_id = uuid4()
    conversation = SimpleNamespace(id=uuid4(), thread_id="thread-1")
    db = SimpleNamespace(commit=AsyncMock(), rollback=AsyncMock())
    ...
    with (
        ...
        conversation_repo_cls.return_value.create_conversation = AsyncMock(return_value=conversation)
        service_cls.return_value.chat = AsyncMock(
            side_effect=AgentRouterUnavailableError("model timeout", "AGENT_LLM_TIMEOUT")
        )
        client = AsyncClient(transport=ASGITransport(app=app), base_url="http://testserver")
        try:
            response = await client.post(
                "/api/agent/chat",
                json={"message": "kiểm UNet", "incomingMessageId": "msg-router-error"},
            )
        finally:
            await client.aclose()
            app.dependency_overrides.clear()
    ...
    body = response.json()
    assert response.status_code == 200
    assert body["fallback"]["errorCode"] == "AGENT_LLM_TIMEOUT"
